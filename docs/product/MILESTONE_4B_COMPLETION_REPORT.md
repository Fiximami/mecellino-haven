...
**Milestone:** 4B — Supabase authentication foundation  
**Branch:** `feat/ydg-mvp`  
...
- In-memory lockout (`LOCKOUT_SCOPE = local_single_process`) — not shared across processes or restarts
...
6. Independent inspection of any external database that ever applied the historical booking migration — editing the source file does not remediate a deployed database
...
| Phase 1 limitations | `docs/PHASE1_LIMITATIONS.md` — booking baseline wording aligned with the neutralized repository migration |
...
### Must not be treated as 4B success — confirmed absent
...
| Mobile navigation at 390×844 | Menu opens; drawer links present |
| `/contact` overflow at 390px | `scrollWidth = clientWidth = 390` |
| Enquiry demonstration | Demonstration wording and “Show demonstration result” present |
