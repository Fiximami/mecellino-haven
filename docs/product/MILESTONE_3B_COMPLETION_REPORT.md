...
# Milestone 3B â€” final report

**Project:** Mecellino Haven / Youth Discovery Gateway  
**Milestone:** 3B â€” public journey and communication synchronization, plus correction pass  
...
- Consent bands 10â€“17 (guardian consent + participant assent) and 18â€“25 (own consent + acknowledgement)
...
1. **Mobile screenshot clipping.** The live interface at 390 CSS pixels does not overflow. Previous mobile PNGs were 390Ã—844 device-pixel crops of a larger bitmap, which removed the right edge (including the menu button and wrapped words). Capture now uses an explicit 390Ã—844 CSS viewport and device scale factor 1, with no post-capture crop or resize. No CSS layout change was required.
...
Viewport: 390Ã—844 CSS pixels, device scale factor 1, production server.
...
- Method: Playwright, explicit viewport 390Ã—844, device scale factor 1 (not an iPhone device preset)
- No browser zoom, no post-capture crop, no resize
- Output files are 390Ã—844 pixels
...
| `/attractions`, `/events`, `/visit` | 307 â†’ `/mobile-amusement` |
| `/gallery` | 307 â†’ `/` |
...
| Mobile screenshots | Seven files regenerated at 390Ã—844, unclipped |
